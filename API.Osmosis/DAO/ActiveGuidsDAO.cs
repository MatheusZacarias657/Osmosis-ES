...
namespace Osmosis.DAO
{
    public class ActiveGuidsDAO : BaseDAO<ActiveGuids>
    {
        ...
        public ActiveGuids FindSessionByGuid (string guid)
        {
            try
            {
                ActiveGuids session = _datacontext.ActiveGuids.Where(x => x.guid.Equals(guid) && DateTime.Now < x.expirationDate).FirstOrDefault();
        ...
        public void RemoveSessionsByUser(int id)
        {
            try
            {
                ...
                sessions.ForEach(x => x.expirationDate = DateTime.Now);
